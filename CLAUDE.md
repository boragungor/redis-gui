...
- Some Redis values are **Java-serialized** (`java.io.Serializable`), not JSON — e.g. `pegaCache_*` holds an `ArrayList` of `GetNbaResponseDto`. `lib/java-value.ts` detects the stream header (`0xACED0005`) and decodes them to JSON-safe data for display. Two rules: the `get` route must use **`client.getBuffer()`**, never `client.get()` (UTF-8 decoding destroys binary bytes irrecoverably), and decoding is **one-way** — the `set` route rejects writes over Java-serialized keys with a 409, because the displayed JSON is not the stored format and saving it would corrupt the key for the Java services reading it.
